...
// このクラスのインスタンスは DataManager.gameMap で参照されます。
//==============================================================================
...
module ebi {
    export module rpg {
        export class Game_Map {
            ...
            private tileEvents_: any[];
            private commonEvents_: any[];
            ...
            private setupEvents(): void {
                ...
                // Game_ event
                _.each(this.map_.events, (event, i) => {
                    this.events_[i] = null;// TODO new Game_Event(this.mapId_, event);
                });

                // Common event
                this.commonEvents_ = _.collect(
                    this.parallelCommonEvents(), 
                    (commonEvent) => null// TODO new Game_CommonEvent(commonEvent.id);
                );
            ...
            public parallelCommonEvents(): any {
                return _.select(DataManager.dataCommonEvents, (event) => event && event.isParallel());
            }
            //--------------------------------------------------------------------------
            // ● スクロールのセットアップ
            //--------------------------------------------------------------------------
            ...
            private setupParallax(): void {
                ...
                this.parallaxSx_ = this.map_.parallaxSx;
                this.parallaxSy_ = this.map_.parallaxSy;
                this.parallaxX_ = 0
                this.parallaxY_ = 0
            }
            //--------------------------------------------------------------------------
            ...
            public setDisplayPos(x: number, y: number): void {
                if (!this.isLoopHorizontal) {
                    x = Math.max(0, Math.min(x, this.width - this.screenTileX));
                }
                if (!this.isLoopVertical) {
                    y = Math.max(0, Math.min(y, this.height - this.screenTileY));
                }
                this.displayX_ = (x + this.width) % this.width;
                this.displayY_ = (y + this.height) % this.height;
                this.parallaxX_ = x
                this.parallaxY_ = y
            }
            //--------------------------------------------------------------------------
            ...
            public parallaxOx(bitmap: any): number {
                if (this.parallaxLoopX_) {
                    ...
                    var w1: number = Math.max(bitmap.width - game.Graphics.width, 0);
                    var w2: number = Math.max(this.width * 32 - game.Graphics.width, 1);
            ...
            public parallaxOy(bitmap: any): number {
                if (this.parallaxLoopY_) {
                    ...
                    var h1: number = Math.max(bitmap.height - game.Graphics.height, 0);
                    var h2: number = Math.max(this.height * 32 - game.Graphics.height, 1);
            ...
            // ● タイルセットの取得
            //--------------------------------------------------------------------------
            public get tileset(): any {
                return DataManager.dataTilesets[this.tilesetId_];
            }
            //--------------------------------------------------------------------------
            // ● 表示名の取得
            //--------------------------------------------------------------------------
            ...
            public get screenTileX(): number {
                return game.Graphics.width / 32;
            }
            //--------------------------------------------------------------------------
            // ● 画面の縦タイル数
            //--------------------------------------------------------------------------
            public get screenTileY(): number {
                return game.Graphics.height / 32;
            }
            //--------------------------------------------------------------------------
            // ● 表示座標を差し引いた X 座標の計算
            //--------------------------------------------------------------------------
            ...
            public roundX(x: number): number {
                return this.isLoopHorizontal ? (x + this.width) % this.width : x
            }
            //--------------------------------------------------------------------------
            // ● ループ補正後の Y 座標計算
            //--------------------------------------------------------------------------
            public roundY(y: number): number {
                return this.isLoopVertical ? (y + this.height) % this.height : y;
            }
            //--------------------------------------------------------------------------
            ...
            // ● 指定座標に存在するイベントの配列取得
            //--------------------------------------------------------------------------
            public eventsXY(x: number, y: number): any[] {
                return _.select(
                    _.values(this.events_), 
            ...
            public eventIdXY_(x: number, y: number): number {
              var list: any[] = this.eventsXY(x, y);
            ...
            public checkPassage(x: number, y: number, bit: number): bool {
                ...
                for (var i = 0; i < tileIds.length; i++) {
                    ...
                    var flag: number = this.tileset.flags[tileId];
                    if ((flag & 0x10) != 0) {
                        continue;     // [☆] : 通行に影響しない
                    }
                    if ((flag & bit) == 0) {
            ...
            //--------------------------------------------------------------------------
            public allTiles(x: number, y: number): any[] {
                var tileEvents: any[] = _.collect(this.tileEventsXY(x, y), (event) => event.tileId);
                var layerdTiles: any[] = this.layeredTiles(x, y);
            ...
            public isLayeredTilesFlag(x, y, bit): bool {
               return  _.any(this.layeredTiles(x, y), (tileId: number) => 
                    this.tileset.flags[tileId] & bit != 0
                );
            }
            //--------------------------------------------------------------------------
            // ● 梯子判定
            ...
            public isLadder(x: number, y: number): bool {
                return this.isValid(x, y) && this.isLayeredTilesFlag(x, y, 0x20)
            }
            //--------------------------------------------------------------------------
            // ● 茂み判定
            //--------------------------------------------------------------------------
            public isBush(x: number, y: number): bool {
                return this.isValid(x, y) && this.isLayeredTilesFlag(x, y, 0x40)
            }
            //--------------------------------------------------------------------------
            // ● カウンター判定
            //--------------------------------------------------------------------------
            public isCounter(x: number, y: number): bool {
                return this.isValid(x, y) && this.isLayeredTilesFlag(x, y, 0x80)
            }
            //--------------------------------------------------------------------------
            // ● ダメージ床判定
            //--------------------------------------------------------------------------
            public isDamagerFloor(x: number, y: number): bool {
                return this.isValid(x, y) && this.isLayeredTilesFlag(x, y, 0x100);
            }
            //--------------------------------------------------------------------------
            // ● 地形タグの取得
            //--------------------------------------------------------------------------
            public terrainTag(x: number, y: number): number {
                ...
                _.every(this.layeredTiles(x, y), (tileId:number) => {
                    tag = this.tileset.flags[tileId] >> 12;
                    return tag == 0;
                })
            ...
            public regionId(x: number, y: number): number {
                return this.isValid(x, y) ? this.map_.data[x, y, 3] >> 8 : 0;
            }
            //--------------------------------------------------------------------------
            // ● スクロールの開始
            //--------------------------------------------------------------------------
            ...
            public updateParallax(): void {
                if (this.parallaxLoopX_) {
                    this.parallaxX_ += this.parallaxSx_ / 64.0;
                }
                if (this.parallaxLoopY_) {
                    this.parallaxY_ += this.parallaxSy_ / 64.0;
                }
            }
            //--------------------------------------------------------------------------
            // ● タイルセットの変更
            ...
            // ● 起動中イベントのセットアップ
            //--------------------------------------------------------------------------
            public setupStartingEvent(): bool {
                if (this.needRefresh) {
                    this.refresh();
                }
            ...
            public isAnyEventStarting(): bool {
                return _.any(_.values(this.events), (event) => event.isStarting());
            }
            //--------------------------------------------------------------------------
            // ● 起動中のマップイベントを検出／セットアップ
            //--------------------------------------------------------------------------
            public setupStartingMapEvent(): any {
                // TODO Game__Event
            ...
            public setupAutorunCommonEvent(): any {
                ...
                var event:any = _.find(DataManager.dataCommonEvents, (event)=> 
                    event && event.isAutoRun() && DataManager.gameSwitches[event.switchId];
                );
